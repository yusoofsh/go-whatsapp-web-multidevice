...
func RecordMCPEvent(ctx context.Context, device, kind, chat, id string) {
	s := mcpEventStore.Load()
	if s == nil || device == "" {
		return
	}
	bounded, cancel := context.WithTimeout(context.WithoutCancel(ctx), 5*time.Second)
	defer cancel()
	if _, err := s.Append(bounded, mcpstore.Event{DeviceID: device, Type: kind, ChatJID: chat, MessageID: id}); err != nil {
		logrus.Errorf("MCP event journal write failed: %v", err)
	}
}
func recordMCPProtocolEvent(ctx context.Context, device string, raw any) {
	...
	case *events.Receipt:
		if e.Sender.Device != 0 {
			return
		}
		for _, id := range e.MessageIDs {
			RecordMCPEvent(ctx, device, "message.receipt", e.Chat.ToNonAD().String(), id)
		}
