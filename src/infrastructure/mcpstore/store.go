...
func Open(path string) (*Store,error) {
 ...
 f,err:=os.OpenFile(path,os.O_CREATE|os.O_RDWR,0600);if err!=nil{return nil,err};_ = f.Close()
 db,err:=sql.Open(sqlite.DriverName,sqlite.FormatChatStorageURI(path,true,true));if err!=nil{return nil,err}
 ...
 if err=s.Prune(context.Background());err!=nil{_ = db.Close();return nil,err}
 return s,nil
}
func(s *Store)Close()error{return s.db.Close()}

func ValidateMedia(filename,mediaType string,data []byte,max int)(string,error){
 if filename=="" || len(filename)>160 || strings.ContainsAny(filename,"/\\") || filename=="." || filename==".." || strings.ContainsFunc(filename,unicode.IsControl){return "",errors.New("invalid attachment filename")}
 if len(data)==0 || len(data)>max{return "",fmt.Errorf("attachment must contain 1..%d bytes",max)}
 if len(mediaType)>120{return "",errors.New("invalid MIME type")}
 parsed,_,err:=mime.ParseMediaType(mediaType);if err!=nil || !strings.Contains(parsed,"/"){return "",errors.New("invalid MIME type")}
 parsed=strings.ToLower(parsed)
 sniff:=http.DetectContentType(data)
 if activeMIME(parsed) || activeMIME(strings.Split(sniff,";")[0]) {return "",errors.New("active HTML, XML, SVG or script attachments are not accepted")}
 if strings.HasPrefix(parsed,"image/") && strings.Split(sniff,";")[0]!=parsed{return "",errors.New("image MIME does not match its bytes")}
 return parsed,nil
}
...
func(s *Store)Stage(ctx context.Context,device,filename,mediaType string,data []byte)(Media,error){
 if device==""{return Media{},errors.New("missing device scope")}
 mediaType,err:=ValidateMedia(filename,mediaType,data,s.maxMediaBytes);if err!=nil{return Media{},err}
 ...
 tx,err:=s.db.BeginTx(ctx,nil);if err!=nil{return Media{},err};defer tx.Rollback()
 if _,err=tx.ExecContext(ctx,"DELETE FROM mcp_media WHERE expires<=?",s.now().Unix());err!=nil{return Media{},err}
 var total,count int64
 if err=tx.QueryRowContext(ctx,"SELECT COALESCE(SUM(size),0),COUNT(*) FROM mcp_media").Scan(&total,&count);err!=nil{return Media{},err}
 if total+m.Size>s.mediaBudget || count>=1024{return Media{},errors.New("MCP attachment quota reached; wait for staged attachments to expire")}
 _,err=tx.ExecContext(ctx,"INSERT INTO mcp_media VALUES(?,?,?,?,?,?,?,?)",m.ID,device,m.Filename,m.MIME,m.Size,m.SHA256,m.ExpiresAt,data)
 if err!=nil{return Media{},err};if err=tx.Commit();err!=nil{return Media{},err};return m,nil
}
func(s *Store)Media(ctx context.Context,device,id string)(Media,[]byte,error){
 if device==""{return Media{},nil,errors.New("missing device scope")}
 if _,err:=uuid.Parse(id);err!=nil{return Media{},nil,errors.New("invalid media_id")}
 var m Media;var data []byte
 ...
 if errors.Is(err,sql.ErrNoRows){return Media{},nil,errors.New("attachment not found for this device or expired")}
 if err!=nil{return Media{},nil,err}
 m.URI="whatsapp://media/"+m.ID;return m,data,nil
}

func(s *Store)Append(ctx context.Context,e Event)(Event,error){
 if e.DeviceID=="" || e.Type=="" || len(e.DeviceID)>256 || len(e.Type)>64 || len(e.ChatJID)>256 || len(e.MessageID)>256{return Event{},errors.New("invalid event metadata")}
 e.Timestamp=s.now().Unix()
 s.mu.Lock()
 if s.now().Sub(s.lastPrune)>=time.Hour {if err:=s.pruneLocked(ctx);err!=nil{s.mu.Unlock();return Event{},err}}
 ...
 if err!=nil{s.mu.Unlock();return Event{},err};e.ID,err=r.LastInsertId()
 listeners:=make([]func(Event),0,len(s.listeners));for _,fn:=range s.listeners{listeners=append(listeners,fn)}
 if err==nil && e.ID%1024==0 {err=s.pruneLocked(ctx)}
 s.mu.Unlock()
 if err!=nil{return Event{},err}
 for _,fn:=range listeners{fn(e)}
 return e,nil
}
func(s *Store)Events(ctx context.Context,device string,after int64,limit int)(EventPage,error){
 ...
 if device=="" || after<0 || limit<1 || limit>500{return p,errors.New("device, nonnegative cursor and limit 1..500 required")}
 s.mu.Lock();var pruneErr error
 if s.now().Sub(s.lastPrune)>=time.Hour{pruneErr=s.pruneLocked(ctx)}
 s.mu.Unlock();if pruneErr!=nil{return p,pruneErr}
 tx,err:=s.db.BeginTx(ctx,&sql.TxOptions{ReadOnly:true});if err!=nil{return p,err};defer tx.Rollback()
 err=tx.QueryRowContext(ctx,"SELECT purged_through FROM mcp_event_watermarks WHERE device_id=?",device).Scan(&p.PurgedThrough)
 if err!=nil && !errors.Is(err,sql.ErrNoRows){return p,err}
 ...
 for rows.Next(){var e Event;if err=rows.Scan(&e.ID,&e.DeviceID,&e.Type,&e.ChatJID,&e.MessageID,&e.Timestamp);err!=nil{_ = rows.Close();return p,err};p.Events=append(p.Events,e)}
 err=rows.Err();_ = rows.Close();if err!=nil{return p,err}
 if len(p.Events)>limit{p.HasMore=true;p.Events=p.Events[:limit]}
 if len(p.Events)>0{p.NextCursor=p.Events[len(p.Events)-1].ID}
 return p,tx.Commit()
}
func(s *Store)Listen(fn func(Event))func(){
 id:=uuid.NewString();s.mu.Lock();s.listeners[id]=fn;s.mu.Unlock()
 return func(){s.mu.Lock();delete(s.listeners,id);s.mu.Unlock()}
}
func(s *Store)Prune(ctx context.Context)error{s.mu.Lock();defer s.mu.Unlock();return s.pruneLocked(ctx)}
func(s *Store)pruneLocked(ctx context.Context)error{
 ...
 if _,err=tx.ExecContext(ctx,"DELETE FROM mcp_events WHERE "+old,cutoff);err!=nil{return err}
 if _,err=tx.ExecContext(ctx,"DELETE FROM mcp_media WHERE expires<=?",s.now().Unix());err!=nil{return err}
 if err=tx.Commit();err==nil{s.lastPrune=s.now()};return err
}
