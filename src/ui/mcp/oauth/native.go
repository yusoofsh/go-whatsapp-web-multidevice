...
func(s *Server)AuthenticateHTTP(ctx context.Context,header string,basic CredentialValidator)(string,error){
 ...
 case "bearer":
  ...
  if err!=nil{return "",errors.New("invalid OAuth access token")}
 ...
 case "basic":
  ...
  if !ok || basic==nil || !basic(username,password){return "",errors.New("invalid Basic credentials")}
  return "basic:"+username,nil
 default:return "",errors.New("authentication required")
 }
}
func(s *Server)HTTPChallenge()string{return fmt.Sprintf(`Bearer resource_metadata="%s", scope="%s"`,s.protectedResourceMetadataURL(),defaultScope)}
