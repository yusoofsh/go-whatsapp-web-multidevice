...
func ValidatePrivateDataDir(dataDir, publicDir string) error {
	if dataDir == "" {
		return errors.New("private MCP data directory required")
	}
	...
	public, err := filepath.Abs(publicDir)
	if err != nil {
		return err
	}
	...
	if rel == "." || (rel != ".." && !strings.HasPrefix(rel, ".."+string(filepath.Separator))) {
		return errors.New("MCP_DATA_DIR must not be inside public statics")
	}
	return nil
}
