...
func dataDevice(ctx context.Context) (string, error) {
	...
	if !ok || d == nil || d.JID() == "" {
		return "", errors.New("select a paired device to access MCP data")
	}
	return d.JID(), nil
}
func registerDataTools(s *server.MCPServer, data *mcpstore.Store, resolver deviceResolver) {
 ...
 "allOf":[{"if":{"properties":{"action":{"const":"upload"}}},"then":{"required":["data_base64","filename","mime_type"]}},{"if":{"properties":{"action":{"const":"read"}}},"then":{"required":["media_id"]}}]}`)))
	...
	s.AddTool(tool, func(ctx context.Context, r mcpg.CallToolRequest) (*mcpg.CallToolResult, error) {
		ctx, _, err := resolveDeviceContext(ctx, r, resolver)
		if err != nil {
		...
		case "upload":
			...
			if len(encoded) > base64.StdEncoding.EncodedLen(mcpstore.MaxMediaBytes) {
				return mcpg.NewToolResultError("attachment exceeds 10 MiB"), nil
			}
			raw, err := base64.StdEncoding.Strict().DecodeString(encoded)
			if err != nil {
				return mcpg.NewToolResultError("invalid base64 attachment"), nil
			}
		...
		case "read":
			m, raw, err := data.Media(ctx, device, r.GetString("media_id", ""))
			if err != nil {
				return mcpg.NewToolResultError(err.Error()), nil
			}
			return mediaResult(m, raw, true, r.GetBool("include_data", false)), nil
		default:
			return mcpg.NewToolResultError("unknown media action"), nil
		}
	})
	...
	s.AddTool(events, func(ctx context.Context, r mcpg.CallToolRequest) (*mcpg.CallToolResult, error) {
		...
		page, err := data.Events(ctx, device, int64(r.GetInt("cursor", 0)), r.GetInt("limit", 100))
		if err != nil {
			return mcpg.NewToolResultError(err.Error()), nil
		}
	...
	s.AddResource(mcpg.NewResource(mcpstore.EventResource, "WhatsApp events", mcpg.WithMIMEType("application/json")), func(ctx context.Context, r mcpg.ReadResourceRequest) ([]mcpg.ResourceContents, error) {
		...
		page, err := data.Events(ctx, device, 0, 100)
		if err != nil {
			return nil, err
		}
	...
	s.AddResourceTemplate(mcpg.NewResourceTemplate("whatsapp://media/{media_id}", "Private attachment"), func(ctx context.Context, r mcpg.ReadResourceRequest) ([]mcpg.ResourceContents, error) {
		...
		m, raw, err := data.Media(ctx, device, strings.TrimPrefix(r.Params.URI, "whatsapp://media/"))
		if err != nil {
			return nil, err
		}
...
func mediaResult(m mcpstore.Media, raw []byte, inline, structuredBytes bool) *mcpg.CallToolResult {
	...
	if structuredBytes {
		...
		}{m, base64.StdEncoding.EncodeToString(raw)}
	}
	...
	if inline {
		if strings.HasPrefix(m.MIME, "image/") {
			out.Content = append(out.Content, mcpg.ImageContent{Type: "image", MIMEType: m.MIME, Data: base64.StdEncoding.EncodeToString(raw)})
		} else {
			out.Content = append(out.Content, mcpg.EmbeddedResource{Type: "resource", Resource: mediaBlob(m, raw)})
		}
	}
	return out
}
func (h *MessageHandler) importDownload(ctx context.Context, r mcpg.CallToolRequest, resp domainMessage.DownloadMediaResponse) (*mcpg.CallToolResult, error) {
	...
	dir := filepath.Join(config.McpDataDir, "downloads")
	rel, err := filepath.Rel(dir, resp.FilePath)
	if err != nil {
		return mcpg.NewToolResultError("invalid download location"), nil
	}
	root, err := os.OpenRoot(dir)
	if err != nil {
		return mcpg.NewToolResultError("cannot access private download"), nil
	}
	defer root.Close()
	f, err := root.Open(rel)
	if err != nil {
		return mcpg.NewToolResultError("download outside private directory"), nil
	}
	defer f.Close()
	defer root.Remove(rel)
	info, err := f.Stat()
	if err != nil || !info.Mode().IsRegular() || info.Size() > mcpstore.MaxMediaBytes {
		return mcpg.NewToolResultError("invalid or oversized download"), nil
	}
	raw, err := io.ReadAll(io.LimitReader(f, mcpstore.MaxMediaBytes+1))
	if err != nil {
		return mcpg.NewToolResultError(err.Error()), nil
	}
	mt := mime.TypeByExtension(filepath.Ext(resp.Filename))
	if mt == "" {
		mt = http.DetectContentType(raw)
	}
	m, err := h.data.Stage(ctx, device, resp.Filename, mt, raw)
	if err != nil {
		return mcpg.NewToolResultError(err.Error()), nil
	}
...
func stagedUpload(ctx context.Context, r mcpg.CallToolRequest, data *mcpstore.Store, kind string) (*multipart.FileHeader, func(), error) {
	noop := func() {}
	id := r.GetString("media_id", "")
	if id == "" {
		return nil, noop, nil
	}
	field := map[string]string{"image": "image_url", "video": "video_url", "audio": "audio_url", "document": "file_url", "sticker": "sticker_url"}[kind]
	if field == "" || r.GetString(field, "") != "" {
		return nil, noop, errors.New("use exactly one of media_id or the matching URL")
	}
	if data == nil {
		return nil, noop, errors.New("attachment storage unavailable")
	}
	device, err := dataDevice(ctx)
	if err != nil {
		return nil, noop, err
	}
	m, raw, err := data.Media(ctx, device, id)
	if err != nil {
		return nil, noop, err
	}
	prefix := map[string]string{"image": "image/", "video": "video/", "audio": "audio/", "sticker": "image/"}[kind]
	if prefix != "" && !strings.HasPrefix(m.MIME, prefix) {
		return nil, noop, errors.New("attachment MIME does not match send type")
	}
	var b bytes.Buffer
	w := multipart.NewWriter(&b)
	header := make(textproto.MIMEHeader)
	header.Set("Content-Disposition", mime.FormatMediaType("form-data", map[string]string{"name": "file", "filename": m.Filename}))
	header.Set("Content-Type", m.MIME)
	...
	form, err := multipart.NewReader(&b, w.Boundary()).ReadForm(mcpstore.MaxMediaBytes + 4096)
	if err != nil {
		return nil, noop, err
	}
